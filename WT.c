...
int main() {
    ...
    // Check if the cache line being replaced has a dirty bit set
    int replaced_index = (address % CACHE_SIZE);
    if (dirty_bits[replaced_index] == 1) {
        // Simulate write-back to memory
        printf("Writing back cache line with tag %d to memory\n", address);
        // Write cache data back to memory address 'address'
        strcpy(memory[address].data, cache[replaced_index].data);
        dirty_bits[replaced_index] = 0; // Clear the dirty bit
    }
    break;
